...
# - Turnover 패널티: TC(0.001)와 동일한 스케일로 맞춰 이중 패널티가 과도해지지 않게 함.
#   TC는 수익률에서 차감(turnover × 0.001), 이 항은 보상에서 추가 차감(turnover × 0.001).
#   전량 교체(turnover≈2) 기준 총 패널티 ≈ 0.004 수준 — 에이전트가 완전히 리밸런싱을
#   포기하지 않을 정도로 적당히 억제하는 수준.
ROLLING_SHARPE_COEF  = 0.002
EXCESS_RETURN_COEF   = 0.3
TURNOVER_REWARD_COEF = 0.001   # 0.002 → 0.001: TC(0.001)와 동일 스케일로 조정
...
class PortfolioEnv(gym.Env):
    ...
    def __init__(
        ...
        # 주의 3 FIX: SPY_ret가 반드시 존재하는지 명시적으로 검증
        assert "SPY_ret" in self.df.columns, (
            "SPY_ret column missing from df_period. "
            "fit_fold_features() 반환 DataFrame에 SPY_ret가 포함되어 있는지 확인하세요."
        )
        ...
        # 주의 1 FIX: __init__ 안에서 reset()을 호출하지 않음.
        # DummyVecEnv가 내부적으로 reset()을 한 번 더 호출하므로
        # 이중 초기화가 발생한다. 대신 상태 변수를 직접 설정한다.
        self.t = 0
...
def run_walk_forward(df_raw: pd.DataFrame) -> tuple[pd.Series, pd.DataFrame]:
    ...
    while train_end + pd.DateOffset(years=TEST_YEARS) <= end:
        ...
        # 버그 1 FIX: lambda는 변수 이름을 캡처하므로, 루프가 돌아 df_train 등이
        # 다음 fold 값으로 바뀌면 엉뚱한 데이터로 환경이 만들어진다.
        # 기본 인자(d=df_train, ...)로 현재 값을 즉시 캡처해서 고정한다.
        train_env = DummyVecEnv([
            lambda d=df_train, s=state_cols, t=turb_threshold: Monitor(
                PortfolioEnv(d, s, turbulence_threshold=t)
            )
        ])
